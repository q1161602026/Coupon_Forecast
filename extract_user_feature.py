...
def extract_user_feature(feature_file_path,num):
	...
	# 用户核销优惠券中的平均、最大、最小、最多距离
	t4,t5,t6,t7 = user_distances(feature)

	# 用户交易过的商家数提取
	t8 = user_merchants_count(feature)
	
	#用户核销天数提取
	t9,t10,t11,t12=user_datereceived_days(feature)
...
# 	t12 = user_each_rate_discount_rate(feature)
	...
	user = pd.merge(user,t4,on=['User_id'],how='left')
	...
	user.user_merchants_count = user.user_merchants_count.replace(np.nan,0)

	# 用户使用优惠券交易率
	user.loc[:,'user_buy_use_coupon_rate'] = user.apply(cal_user_buy_use_coupon_rate,axis=1)
	
	# 用户使用优惠券率
	user.loc[:,'user_use_coupon_rate'] = user.apply(cal_user_use_coupon_rate,axis=1)
