...
def cal_user_merchant_buy_use_coupon_trade_rate(x):
	# 用户领取核销该商家优惠券占交易的比例
	return 1.0*x['user_merchant_buy_use_coupon_count']/x['user_merchant_trade_count']

def cal_user_merchant_buy_notuse_coupon_trade_rate(x):
	# 用户不使用该商家优惠券占交易的比例
	return 1.0*x['user_merchant_buy_notuse_coupon_count']/x['user_merchant_trade_count']

def cal_user_merchant_trade_rate(x):
	# 用户与该商家交易占所有关联的比例
	return 1.0*x['user_merchant_trade_count']/x['user_merchant_count']
...
def cal_user_buy_use_coupon_rate(x):
	return 1.0*x['user_buy_use_coupon_count']/x['user_buy_count']
...
def user_merchant_buy_notuse_coupon_count(feature):

	t1 = feature[(feature.Date!='null')&(feature.Date_received=='null')][['User_id','Merchant_id']]
	t1['user_merchant_buy_notuse_coupon_count'] = 1
	t1 = t1.groupby(['User_id','Merchant_id']).agg('sum').reset_index()
	print '用户不使用商家优惠券交易次数提取'
	return t1
...
def merchant_datereceived_days(feature):
	...
	t11.rename(columns={'merchant_datereceived_days':'merchant_mean_datereceived_days'},inplace=True)
	# t11.merchant_mean_datereceived_days = t11.merchant_mean_datereceived_days.apply(round,2)

	t12 = t10.agg('min').reset_index()
	t12.rename(columns={'merchant_datereceived_days':'merchant_min_datereceived_days'},inplace=True)

	t13 = t10.agg('max').reset_index()
	t13.rename(columns={'merchant_datereceived_days':'merchant_max_datereceived_days'},inplace=True)

	t14 = t10.agg('median').reset_index()
	t14.rename(columns={'merchant_datereceived_days':'merchant_median_datereceived_days'},inplace=True)
